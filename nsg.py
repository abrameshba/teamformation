...
def nsga2(population_size, num_generations, crossover_prob, mutation_prob, task):
    import time
    start = time.time_ns()
    population = initialize_population(population_size, task)
    population = unique_teams(population)        # code for removing duplicate
    # # Convert back to lists if needed
    # population = [list(x) for x in unique_lists]
    for generation in range(num_generations):
        offspring = []
        while len(offspring) < population_size:
            if len(population) < 2:
                return population, time.time_ns() - start, [(gamma_diversity(graph, ind, task), sum_distance(graph, ind, task)) for ind in population]
            else:
        ...
        fitness_values = [(gamma_diversity(graph, ind, task), sum_distance(graph, ind, task)) for ind in population]
        population = selection(population, fitness_values, population_size)
    fronts = non_dominated_sorting(population, fitness_values)
    # Return the final population and their fitness values
    final = [population[i] for i in fronts[0]]
    lst = [(gamma_diversity(graph, ind, task), sum_distance(graph, ind, task)) for ind in final]
    return final, time.time_ns() - start, lst
...
for network in networks:
    ...
    for task in tasks:
        i += 1
        final_population, ptime, final_fitness = nsga2(population_size, num_generations, crossover_prob,
                                                       mutation_prob, task)
        with open("/home/ramesh/dblp/output/" + network + "_nsga2_teams_all.txt", "a") as file:
            ...
            if i % 5 == 0:
                file.write("\n")
        with open("/home/ramesh/dblp/output/" + network + "_nsga2_fitness.txt", "a") as file1:
...
def analysis():
    ...
    for network in networks:
        print(network)
        open("/home/ramesh/dblp/output/" + network + "_" + "nsga2_teams.txt", "w").close()
        ab = set()
        with open("/home/ramesh/dblp/output/" + network + "_nsga2_teams_all.txt") as file:
            i = 0
            # ptime = 0
            for line in file:
                words = line.strip("\n").split("\t")
                ...
                with open("/home/ramesh/dblp/output/" + network + "_nsga2_teams.txt", "a") as wfile:
                    if len(words) > 1 and words[0] not in ab:
                        ...
                        # ptime += int(words[0])
                        # if i % 5 == 0:
                        #     with open("/home/ramesh/dblp/output/" + network + "_nsga2_analysis.txt", "a") as w2file:
...
#         nsga2_teams = []
#         with open("/home/ramesh/dblp/output/" + network + "_nsga2_teams.txt") as file:
#             for line in file:
#                 result = line.strip("\n").split("\t")
#                 if len(result) > 1:
